...
void fill_OF3F7() {
    opcode_funct3_funct7["add"].set(51,0,0);
    opcode_funct3_funct7["sub"].set(51,0,64);
    opcode_funct3_funct7["sll"].set(51,1,0);
    opcode_funct3_funct7["slt"].set(51,2,0);
    opcode_funct3_funct7["sltu"].set(51,3,0);
    opcode_funct3_funct7["xor"].set(51,4,0);
    opcode_funct3_funct7["srl"].set(51,5,0);
    opcode_funct3_funct7["or"].set(51,6,0);
    opcode_funct3_funct7["and"].set(51,7,0);

    opcode_funct3_funct7["addi"].set(19,0,0);
    opcode_funct3_funct7["slli"].set(19,1,0);
    opcode_funct3_funct7["slti"].set(19,2,0);
    opcode_funct3_funct7["sltui"].set(19,3,0);
    opcode_funct3_funct7["xori"].set(19,4,0);
    opcode_funct3_funct7["srli"].set(19,5,0);
    opcode_funct3_funct7["ori"].set(19,6,0);
    opcode_funct3_funct7["andi"].set(19,7 ,0);

    opcode_funct3_funct7["lui"].set(55,0,0);;

    opcode_funct3_funct7["lw"].set(3,2,0);

    opcode_funct3_funct7["sw"].set(35,2,0);

    opcode_funct3_funct7["beq"].set(99,0,0);
    opcode_funct3_funct7["bne"].set(99,1,0);
    opcode_funct3_funct7["blt"].set(99,4,0);
    opcode_funct3_funct7["bge"].set(99,5,0);
    opcode_funct3_funct7["bltu"].set(99,6,0);
    opcode_funct3_funct7["bgeu"].set(99,7,0);

    opcode_funct3_funct7["j"].set(103,0,0);
    opcode_funct3_funct7["jal"].set(111,0,0);

    opcode_funct3_funct7["jalr"].set(107,0,0);
...
//    memory[3] = 8'h03;
    ...
    cout << "assign add0 = (address & 11'h7ffc) + 11'd0;\n";
    cout << "assign add1 = (address & 11'h7ffc) + 11'd1;\n";
    cout << "assign add2 = (address & 11'h7ffc) + 11'd2;\n";
    cout << "assign add3 = (address & 11'h7ffc) + 11'd3;\n";
...
void R_type(string instruction) {
    ...
    //print for debugging
    //cout << opcode << ' ' << funct3 << ' ' << funct7 << ' ' << rd << ' ' << rs1 << ' ' << rs2 << endl;
...
void U_type(string instruction) { //for lui
    ...
    //cout << to_binary(rd, 5) << ' ' << to_binary(imm, 20) << ' ' << to_binary(opcode, 7) << endl;
...
void S_type(string instruction) { // sw rs2 -100 rs1
    vector<string> tokens;
    ...
    //get the binary representation of the instruction
    string binary = "";
    //get immediate then seperate it into 2 parts imm1 = imm[4:0] and imm2 = imm[11:5]
    string imm_binary = to_binary(imm,12);
    string imm1 = imm_binary.substr(0,5);
    string imm2 = imm_binary.substr(5,7);
    binary += imm1;
    binary += to_binary(rs1,5);
    binary += to_binary(rs2,5);
    binary += imm2;
    ...
    //cout << imm1 << ' ' << to_binary(rs1,5) << ' ' << to_binary(rs2,5) << ' ' << imm2 << ' ' << to_binary(funct3,3) << ' ' << to_binary(opcode,7) << endl;
...
void J_type(string instruction) { // j 1000, jal 1000
    vector<string> tokens;
    //v[0] = mnemonic, v[1] = imm
    int i = 0;
    while(i < instruction.size()) {
        string temp = "";
        while(i < instruction.size() && instruction[i] != ' ') {
    ...
    OF3F7 of3f7 = opcode_funct3_funct7[tokens[0]];
    int opcode = of3f7.opcode;
    int imm = stoi(tokens[1]);

    //get the binary representation of the instruction
    string binary = "";
    binary += to_binary(imm, 25);
    binary += to_binary(opcode, 7);
